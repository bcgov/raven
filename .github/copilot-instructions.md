...
- **All `server-mcp`, `imis-mcp`, `health-mcp`, `assets-mcp`, `overview-mcp` operations are READ-ONLY** by design. For `imis-mcp`'s `explore_server`, read-only is enforced by the per-command argument policy in `ssh-executor.ts` — allowlisted binaries only, with executing or mutating options (`find -exec`, `sort -o`, `rpm --pipe`, `uniq OUTPUT`, `date -s`, `hostname NAME`, `jstat -J...`) rejected. A command that passes `validateCommand` yet mutates the remote is a bug: flag it.
