...
| `RSEC-005` | **Fixed** | `localGuard` validates Host, Origin and `Sec-Fetch-Site` ahead of every API router; requests require an allowed Origin, the custom client header, or (for reads) same-origin fetch metadata, except the read-only cached health probe |
